function call_create_pool_ripples(JND,subject,ripple_dir,presentation_destination)

fprintf('Preparing the stimuli for you! \n')
fprintf('Thank you to participate in our experiment! \n')

% showfig=0; create_thumbnails_only=1;
 showfig=0; create_thumbnails_only=0;   slfn='stimulus_list.txt'; fid=fopen([ripple_dir slfn],'w');         

 
% DEFINE the subjects name here, creates a directory with this name, needs
% to be defined in the .tem file in presentation, too.

datadir=[ripple_dir subject];
...
%             subplot(ceil(length(W)/4),ceil(length(W)/4),imidx)
    ...
    sdir=[datadir '/' subject '_parametric_sfiles'];
    ...
    sdir=[datadir '/' subject '_ripple_vel_bin_sfiles'];
    if(~exist(sdir,'dir')), mkdir(sdir);end
...
% this command will create subj directory name and copy ripple_Sounds to
% stimuli folder in presentation
command=['rsync -rvlutz ' sdir ' ' presentation_destination];
