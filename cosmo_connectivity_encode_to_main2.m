...
for iroi1=2:length(lnames)-1
    ...
    for iroi2=iroi1+1:length(lnames)
        ...
        if check1+check2==2
            ...
            if(isempty(vols1)),vols1=zeros(19,nc,nroi1,nroi2,5);vols2=vols1; vols3=vols1; vols4=vols1; end
            
            
            if(isempty(vols1m)),vols1m=zeros(19,nc,nroi1,nroi2,5);vols2m=vols1m; vols3m=vols1m; vols4m=vols1m; end
            ...
            for freqi=1:5
                ...
                vols1(:,:,:,:,freqi)=abs(d1.data);
                vols2(:,:,:,:,freqi)=abs(d2.data);
                vols3(:,:,:,:,freqi)=abs(d3.data);
                vols4(:,:,:,:,freqi)=abs(d4.data);
                
                
                vols1m(:,:,:,:,freqi)=abs(d1m.data);
                vols2m(:,:,:,:,freqi)=abs(d2m.data);
                vols3m(:,:,:,:,freqi)=abs(d3m.data);
                vols4m(:,:,:,:,freqi)=abs(d4m.data);
            ...
            for sidx=1:size(d1.data,1)
                ...
                for fold=1:length(runs)
                    ...
                    samples_train= squeeze(mean(vols(traini(1:3),:,:)));% squeeze([squeeze(vols(traini(1),:,:));squeeze(vols(traini(2),:,:)); squeeze(vols(traini(3),:,:))];
                    samples_test = squeeze(volsm(fold,:,:));
                    
                    tmpacc=zeros(6,6);
                    for i=1:6
                        for j=1:6
                            tmpacc(i,j)=atanh(corr(samples_train(i,:)',samples_test(j,:)'));
                        end
                    end
                    
                    HR(sidx,fold) = mean(diag(tmpacc))-mean(tmpacc(logical(abs(eye(6,6)-1))));
