        ...
        for iroi2=iroi1+1:length(lnames)
            ...
            if check1+check2==2
                ...
                for sidx=1:size(d1.data,1)
                    ...
                    combinations={[1,2;3,4];[1,3;2,4];[1,4;2,3];[3,4;1,2];...
                        [2,4;1,3];[2,3;1,4]};
                    
                    for fold=1:length(combinations)
                        
                        tr = combinations{fold}(1,:);
                        ts = combinations{fold}(2,:);
                        
                        
                        samples_train= squeeze(mean((vols(tr,:,:))));
                        samples_test = squeeze(mean(volsm(ts,:,:)));
                ...
                mvpaRes([6,14])=[];
                [h p ci stat]=ttest(mvpaRes,0,'Tail','right')
...
save([dpath , 'EtoM_corr_run_connectivity_5freq_uncorrected_pvalues.mat'],'p_value');


save_p = [dpath , 'EtoM_corr_connectivity_5freq_uncorrected_pvalues.csv'];
