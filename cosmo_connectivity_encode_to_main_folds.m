                ...
                for sidx=1:size(d1.data,1)
                    ...
                    for fold=1:4
                       ...
                       % samples_train= [squeeze(vols(1,:,:));squeeze(vols(2,:,:)); squeeze(vols(3,:,:));squeeze(vols(4,:,:))];
                        samples_test = squeeze(volsm(ifold,:,:));
...
save([dpath, 'ALL_connectivity_5freq_subjects_accuracy_' num2str(t1) 'to' num2str(t2) 's.mat'],'accuracy','roi_to_roi')
...
clc
fid=fopen([dpath , 'ALL_connectivity_5freq_corrected_pvalues.txt']);
C=textscan(fid,'%s');
corr_p=C{1,1};
...
roi_freq(corrected_p(1:60)<0.05)
