...
% showfig=0; create_thumbnails_only=1;
 showfig=0; create_thumbnails_only=0;   slfn='/autofs/cluster/transcend/fahimeh/Jyrki/doc/stimulus_list.txt'; fid=fopen(slfn,'w');         
...
% create vector of all possible ripple velocity (w) values, in which there
% are Nsteps octave steps between the lowest W0 and highest Wmax possible values. Here,
% lets use a ripple velocity total range of Wrange=4 octaves starting from
% W0=3 cycles/s. Then perhaps Nsteps=24 per octave
Wrange=4;
W0=3; 
Wmax=W0*2.^(Wrange);
Nsteps=24;
...
%             subplot(ceil(length(W)/4),ceil(length(W)/4),imidx)
    ...
    sdir='/autofs/cluster/transcend/fahimeh/Jyrki/resources/ripple_vel_bin_sfiles-1s/';
    if(~exist(sdir,'dir')), mkdir(sdir);end
