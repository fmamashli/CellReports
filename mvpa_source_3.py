    ...
    for idx in range(X.shape[1]):
        ...
        for iteration in range(10):
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                    X_vert, y, test_size=0.2)
            
            n_components=min(X_train.shape[0],26)
            ...
            vr.append(np.sum(v[:n_components]))
            ...
            print('Score for vert %d of %d = %f iteration %d'
                  % (idx, X.shape[1],scores[-1],iteration))
            print('explained variance %f' % (np.array(vr).mean(axis=0)))     
        
        total_variance.append(np.array(vr).mean(axis=0))
        scores_vert.append(np.array(scores).mean(axis=0))
...
#                  np.array_split(X, n_jobs, axis=1))
    ...
    return scores_vert, total_variance
...
for subj in subjects[0:21]:
    ...
    save_file = data_path + subj + '/megdata/stimulus_' + subj + \
    '_aw_source_timeseries_12Hz'
    times = epochs.times
    
    np.savez(save_file, Xl = Xl, Xr = Xr, y = y, times = times)
    
    ind_time = np.where(epochs.times>0)[0]
    
    scores_meanl, lvariance = compute_ml(Xl[:,:,ind_time],y)
    
    scores_meanr, rvariance = compute_ml(Xr[:,:,ind_time],y)
...
#    
    ...
    fname = data_path + subj + '/megdata/stimulus_' + subj + '_0to1s_' + \
    'pca26_aw_decoding_scores'
    ...
    np.savez(save_file, lvariance = lvariance, rvariance = rvariance)
...
#                      clim=dict(kind='value', lims=(0.18, 0.2, 0.25)), size = 600)
#    
#
